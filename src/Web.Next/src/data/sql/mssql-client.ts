...
/**
 * Catalog DB connection from environment only (no hard-coded hosts/secrets).
 * Prefers the names used across LCFM seed/commerce tickets.
 */
export function getCatalogConnectionString(): string | undefined {
  return (
    process.env.CATALOG_SQL_CONNECTION_STRING ??
    process.env.ESHOP_CATALOG_CONNECTION_STRING ??
    process.env.ESHOP_CATALOG_CONNECTION ??
    process.env.CatalogConnection ??
    undefined
  );
}
